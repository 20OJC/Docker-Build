...
# @@@SNIPSTART setup-es-template-commands
    ...
    echo "******************Before url 500 error*********************"
	curl --user "${ES_USER}":"${ES_PWD}" -X PUT "${INDEX_URL}" --write-out "\n"
	echo "******************Setup Index Over ${ES_USER}":"${ES_PWD} ${INDEX_URL}******************"
...
if [[ ${SKIP_SCHEMA_SETUP} != true ]]; then
    validate_db_env
	echo "******************After validate_db_env******************"
    wait_for_db
	echo "******************After wait_for_db******************"
    setup_schema
	echo "******************After setup_schema******************"
fi

if [[ ${ENABLE_ES} == true ]]; then
    validate_es_env
	echo "******************After validate_es_env******************"
    wait_for_es
	echo "******************After wait_for_es******************"
    setup_es_index
	echo "******************After setup_es_index******************"
fi
